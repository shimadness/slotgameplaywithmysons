...
export class DropBoard {
  ...
  private buildOddsPanel(): HTMLElement {
    ...
    panel.appendChild(this.buildOddsRow("gold7"));
    // 以降は弱→強の逆順（red7 が上）。現在オッズのみ表示（「次の倍率」は非表示）
    for (const id of [...BASE_SYMS].reverse()) panel.appendChild(this.buildOddsRow(id));
    return panel;
  }
  private buildOddsRow(id: DSym): HTMLElement {
